...
import com.azu.hospital.accounting.all_item_expanse.Labs.dto.PatientExpanseFinalListLabDtoMapper;
...
@Service
public class GetPatientExpanseFinalListLab implements GetPatientExpanseFinalList {

    private final PatientDao patientDao;
    private final PatientLabExpanseResultTableDao dao;
    private final PatientExpanseFinalListLabDtoMapper mapper;
    private final ExceptionsMessageReturn messageReturn;

    @Autowired
    public GetPatientExpanseFinalListLab(
            PatientDao patientDao,
            PatientLabExpanseResultTableDao dao,
            PatientExpanseFinalListLabDtoMapper mapper,
    ...
    public ExpanseDtoScreen getPatientExpanseByPatientId(Long patientId) {
        ...
        List<PatientLabExpanseResultTable> expansesResultTable = dao.getAllPatientExpansesExpanseResultTableByPatientId(patient.getId());
    ...
    public ExpanseDtoScreen getArchivedPatientExpanseByPatientId(Long patientId) {
        ...
        List<PatientLabExpanseResultTable> expansesResultTable = dao.findAllByPatientIdAndIsArchivedTrue(patient.getId());
        List<ExpanseDto> expanseDto = expansesResultTable
                .stream()
                .map(mapper::getExpanseDtoList)
                .toList();
    ...
    public ExpanseDtoScreen GetPatientExpanseFinalListByIdAndDate(Long patientId, LocalDate date) {
        ...
        List<PatientLabExpanseResultTable> expansesResultTable = dao.findAllByPatientIdAndIsArchivedIsTrueAndByDateOfPayment(patient.getId(), date);
        List<ExpanseDto> expanseDto = expansesResultTable
                .stream()
                .map(mapper::getExpanseDtoList)
                .toList();
